...
class ModelFiles():
    def __init__(self, model_num):
        ...
        self.all_relevant_files = [filename for filename in self.allfiles if re.search(r"(?!mosinit).*(\.hoc|\.py|\.mod|\.m|\.c|\.ode)", filename)]
    ...
    def figfiles(self): # retrieve all files that start with 'fig'
        self.fig_files = [filename for filename in self.allfiles if re.search(r"(^fig).*(\.hoc|\.py|\.m|\.c|\.ode)", filename)]
...
data_structure = { # the data structure GPT should respond in
    "filename": "the name of the file you're analyzing, str",
    "celltypes": [list],
    "genes": [list],
    "modelconcepts": [list],
    "modeltypes": [list],
    "receptors": [list],
    "regions": [list],
    "simenvironments": [list],
    "transmitters": [list],
    "short_model_description": "a short description of the model, str"
}
...
with shelve.open("gpt_data") as data:
    for category, filelist in compilation.items():
        ...
        for filename in tqdm.tqdm(filelist):
            with open(find_file(filename, model.model_directory)) as file:
                content = file.read()
            ...
            data[f'{model_id}/{category}/{filename}'] = response.choices[0].message.content
